#!/usr/bin/env python3
"""Append a fresh Greenhouse scrape into a provider tracking XLSX.
...
def normalize_row(row: dict, today: str, default_provider: str = "greenhouse") -> dict:
    normalized = {col: "" for col in COLUMNS}
    for key, value in row.items():
        target = ALIASES.get(key, key)
        if target in normalized:
            normalized[target] = "" if value is None else str(value)
    normalized["provider"] = normalized["provider"] or default_provider
...
def write_xlsx(path: Path, rows: List[dict]) -> None:
    ...
    for r_idx, row in enumerate(data, start=1):
        ...
        for c_idx, value in enumerate(row):
            ...
            cells.append(
                f'<c r="{ref}" t="inlineStr"><is><t>{xml_text(str(value))}</t></is></c>'
            )
...
def merge(existing: Iterable[dict], fresh: Iterable[dict], today: str) -> List[dict]:
    ...
    for row in existing:
        provider = str(row.get("provider", "")).strip() or "greenhouse"
        normalized = {col: str(row.get(col, "") or "") for col in COLUMNS}
        normalized["provider"] = normalized["provider"] or provider
...
def main() -> int:
    ...
    print(
        json.dumps(
            {
                "total": len(merged),
                "greenhouse": sum(1 for row in merged if row.get("provider") == "greenhouse"),
