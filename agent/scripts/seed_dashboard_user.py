...
Part of the dashboard-auth security feature: the dashboard authenticates
against the PocketBase `users` collection (see dashboard/lib/auth.ts and
dashboard/app/login).
...
Uses the superuser PBClient from agent/scripts/pb_client.py (stdlib only).

Usage:
  python3 agent/scripts/seed_dashboard_user.py
