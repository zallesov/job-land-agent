...
description: Knows how to invoke the Greenhouse scraper script via CDP connection to local Chrome. Use when running the greenhouse scraping step of the daily pipeline.
---

# Greenhouse Scraper (CDP via Local Chrome)

**HARD RULE:** All Greenhouse scraping MUST use `pw.chromium.connect_over_cdp("http://localhost:9222")` to attach to the user's existing Chrome. This inherits the Chrome profile (cookies, auth tokens, session state). There is no `--headless` mode — it's deprecated and ignored.

## Script

```
cd /Users/zall/interviews && python3 scripts/scrape_greenhouse.py
```
...
| `--titles <t1> <t2> ...` | none | Job titles for keyword search; omit for personalised feed |
...
# Keyword search (all titles in one call):
