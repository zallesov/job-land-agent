...
$HERMES_HOME/memories/      Memory files (MEMORY.md, USER.md) — see below
...
```
$HERMES_HOME/memories/MEMORY.md    Agent's durable notes (memory tool)
$HERMES_HOME/memories/USER.md      User profile entries
```

Entries are separated by `§` (section sign) delimiters. These files are:
- **Loaded at session start** by `MemoryStore.load_from_disk()` in `tools/memory_tool.py`
- **Frozen as a snapshot** for the system prompt — never mutated mid-session (keeps prefix cache stable)
- **Written atomically** with file locking (`.lock` sidecar files)
- **Directly readable** — `cat $HERMES_HOME/memories/MEMORY.md` shows everything

The `memory.memory_char_limit` and `memory.user_char_limit` config keys control how much is injected at conversation start (shown as the "MEMORY" and "USER PROFILE" blocks). The underlying files can be larger — the limits only affect what the model sees.

For full internals (class hierarchy, write path, tool routing): `references/memory-storage-internals.md`.
...
| Custom endpoint | Config | `model.base_url` + `model.api_key` in config.yaml. For local servers (LM Studio, Ollama, llama.cpp, vLLM) that don't need auth, set `api_key` to a dummy value — without it Hermes silently falls back to the default provider. |
...
Skills bloat context. Every installed skill (name + description) is injected
into the system prompt each turn. 92 enabled skills ≈ 3,500 tokens burned
before the conversation even starts.

### Profile-Specific Skill Pruning

Specialized profiles (InterviewPrep, coding-agent, etc.) should disable
skills irrelevant to their purpose:

```bash
# Interactive (requires real terminal):
hermes --profile <name> skills config
```

Non-interactive alternative — edit the profile's config.yaml directly
and set `skills_disabled` to a list of skill names to suppress at load
time (still installed, just not injected into the system prompt).

Rule of thumb: if the profile's job is "land software engineering
interviews", then gaming/music/ML-training/smart-home skills should be
off. If a profile only ever writes Python, disable Ruby/Node/Go skills.

See `references/interviewprep-skill-pruning.md` for a worked example
mapping 92 skills → ~30 relevant for an interview-prep agent.

### session_search Context Bloat

`session_search` injects matching messages into the current conversation
as tool output. If the match lands on a context-compaction summary from
a prior session, the entire compaction block (often 3,000+ tokens) becomes
permanent conversation history you cannot remove.

**Pitfall:** The agent may run `session_search` to "get more context" when
the authoritative source already answered the question. The search hits a
compaction block, and suddenly the session is bloated with stale context.

**Rule:** Trust authoritative sources first (`cronjob list`, `hermes config`,
file reads of current state). Only reach for `session_search` when the
answer genuinely cannot be found in current state. If the authoritative
source already says "empty" or "not configured," stop — there's nothing
to find in history.
...
5. **Custom/local provider silently uses default model**: If you configured a custom endpoint or local server (LM Studio, Ollama, llama.cpp, vLLM) with `model.provider`, `model.base_url`, and `model.default` set correctly, but Hermes still connects to a different model (e.g. OpenRouter/DeepSeek) — the `model.api_key` field is likely empty. Hermes requires `api_key` to be set even for local endpoints that don't need authentication (use a dummy value like `not-needed`). Without it, the provider routing silently falls back to the default.
   ```bash
   # Fix:
   hermes config set model.api_key not-needed
   # Then restart Hermes (config changes don't apply to running sessions)
   ```
   Verify the fix with `hermes config` — the Model line should now show `api_key` populated. Also confirm the local endpoint is reachable:
   ```bash
   curl -s http://localhost:1234/v1/models  # LM Studio
   # or: curl -s http://localhost:11434/v1/models  # Ollama
   ```
