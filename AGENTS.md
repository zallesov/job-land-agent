...
```
skills/                   Hermes skills (all skills live here, never in hermes-profile/skills/)
scripts/
...
dashboard/                Next.js dashboard (pnpm run dev → http://localhost:3000)
tests/                    pytest suite
...
| `run-dashboard` | Start Next.js dashboard and open in browser |
...
Next.js app in `dashboard/`. Reads `../jobs.db` (relative to cwd at start time).

**Always start from the active profile root:**
```bash
...
Never start from a developer checkout when testing an installed profile.
...
- `.env` (API keys)
- `hermes-profile/*` (runtime state)
- `sessions/`, `logs/`, `cache/`, `plans/`, `workspace/`
- `dashboard/node_modules/`, `dashboard/.next/`
